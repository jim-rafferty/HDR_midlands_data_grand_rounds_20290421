...
\begin{frame}{Results - Simulation study}
    \begin{center}
        \begin{tabular}{cc}
             \includegraphics[width=0.45\columnwidth]{phi_coverage.png}&  \includegraphics[width=0.45\columnwidth]{gradient_coverage.png}
        \end{tabular}
    \end{center}
\end{frame}
...
\begin{frame}[noframenumbering]{BPR - Model likelihood}

\begin{align}
    p &= p_M . p_R \\
    p_M \left(x_{ij} |z_i=k \right)_{ij} &\sim \mathrm{Bernoulli}\left(\phi_j\right) \\
     p_R(y_i |z_i=k,w_{i})_i  &\sim \mathrm{Bernoulli}\left(Y_i \right) \\
\mathrm{logit}\left(Y_i \right) &= \beta_{0k} + \sum_a \beta_a w_{ia}
\end{align}
Note: Dirichlet process omitted
